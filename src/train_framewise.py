    ...
    def train_step2(self, batch):
        ...
        for i, outputs in enumerate(self.forward_generator_step2(noisy_win_stack[1:, :, :, :, :], 
            ...
            if (i+1) % loss_step == 1:
                ...
                generator_outputs['est_imag'] = torch.stack(generator_outputs['est_imag'], dim=2).squeeze(4)
                generator_outputs['est_mag'] = torch.stack(generator_outputs['est_mag'], dim=3).squeeze(4)
                generator_outputs['clean_real'] = clean_spec[:, 0, st:en, :].unsqueeze(1)
                generator_outputs['clean_imag'] = clean_spec[:, 1, st:en, :].unsqueeze(1)
                generator_outputs['clean_mag'] = torch.sqrt(clean_spec[:, 0, st:en, :]**2 + clean_spec[:, 1, st:en, :]**2).unsqueeze(1)

                loss = self.calculate_generator_loss2(generator_outputs, samples=self.samples)
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()
                print(f"Generator loop step:{i}, Loss:{loss}, RAM:{psutil.vistual_memory().percent}") 
